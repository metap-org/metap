...
describe("generateOpenApiDocument", () => {
  it("generates a document with openapi 3.1.0 and one path group per entity", () => {
    ...
    expect(doc.paths["/api/crm.customers"]).toBeDefined();
    expect(doc.paths["/api/crm.customers/{id}"]).toBeDefined();
  });

  it("includes the exact enum values for an enum field", () => {
    const doc = generateOpenApiDocument([customer]);
    const createOp = doc.paths["/api/crm.customers"]?.post as {
      requestBody: { content: { "application/json": { schema: { properties: { data: { properties: Record<string, { enum?: string[] }> } } } } } };
    };
    const statusSchema =
  ...
  it("generates a transitions path for an entity with a workflow", () => {
    const doc = generateOpenApiDocument([customer]);
    expect(doc.paths["/api/crm.customers/{id}/transitions/{action}"]).toBeDefined();
  });
});
