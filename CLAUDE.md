...
- **`dev-tools`** (binary) — `gen-keys`/`mint-token`/`seed-admin`/`create-user` subcommands, replacing the old `packages/core/scripts/*.mjs`. `mint-token` and `create-user` both call `metap_peripherals::mint_jwt`/`create_user` — the same functions `POST /auth/login` and `POST /admin/users` use — so a CLI-minted token or CLI-seeded user can't diverge from what a real login/admin-provisioned one looks like.
