...
pnpm dev                                  # run API with tsx watch
pnpm worker:outbox                        # run the outbox-publisher worker loop
...
Run a single test file: `pnpm vitest run path/to/file.test.ts`. There is no dedicated test config file (no `vitest.config.ts`); vitest runs with defaults.
...
```
HTTP routes (src/server/routes/*)
  -> application services (src/core/crud/crud-service.ts)
    -> platform core (metadata / permission / query-planner / workflow)
      -> repositories / Drizzle client (src/infra/db)
      -> outbox (src/core/outbox) -> RabbitMQ publisher (src/infra/messaging)
```

Everything is wired together in `src/core/container.ts` (`createContainer`), a plain dependency-injection factory — no framework DI. Routes receive the container and call into services; they never touch Drizzle or RabbitMQ directly. When adding a new service, wire it in the container, not by importing infra directly into a route or another service.
