...
import { testWidgetEntity } from "../../core/__fixtures__/test-widget.entity";
...
describe("admin routes (live DB)", () => {
  ...
  beforeAll(async () => {
    ...
    app = await buildApp(config, [testWidgetEntity]);
  ...
  it("creates a policy, lists it filtered by entity, then deletes it", async (ctx) => {
    ...
    const createResponse = await app.inject({
      ...
      url: "/admin/policies",
      headers: { authorization: `Bearer ${adminToken}` },
      payload: { entity: "test.widgets", action: "update", roles: ["editor"] },
    });

    expect(createResponse.statusCode).toBe(201);
    const created = createResponse.json<{ data: { id: string; entity: string } }>();
    expect(created.data.entity).toBe("test.widgets");

    const listResponse = await app.inject({
      method: "GET",
      url: "/admin/policies?entity=test.widgets",
      headers: { authorization: `Bearer ${adminToken}` },
    });
  ...
  it("rejects a field-scoped policy with an incoherent action", async (ctx) => {
    ...
    const response = await app.inject({
      ...
      url: "/admin/policies",
      headers: { authorization: `Bearer ${adminToken}` },
      payload: { entity: "test.widgets", action: "create", field: "phone" },
    });
  ...
  it("simulates a decision for a hypothetical caller via /admin/policies/explain", async (ctx) => {
    ...
    try {
      const deniedResponse = await app.inject({
        ...
        headers: { authorization: `Bearer ${adminToken}` },
        payload: { roles: ["viewer"], entity: "test.widgets", action: "update" },
      });
      ...
      const passingResponse = await app.inject({
        ...
        url: "/admin/policies/explain",
        headers: { authorization: `Bearer ${adminToken}` },
        payload: { roles: ["editor"], entity: "test.widgets", action: "update" },
      });
  ...
  it("rejects a non-admin caller on /admin/policies/explain", async (ctx) => {
    ...
    const response = await app.inject({
      ...
      url: "/admin/policies/explain",
      headers: { authorization: `Bearer ${nonAdminToken}` },
      payload: { roles: ["admin"], entity: "test.widgets", action: "update" },
    });
