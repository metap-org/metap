...
function buildTestApp(
  ...
  app.get("/protected", (request) => ({ context: request.context }));
...
describe("auth hook", () => {
  ...
  it("rejects a request with no authorization header", async () => {
    const roleAssignments = { getRolesForUser: () => Promise.resolve([]) };
  ...
  it("attaches request context with roles resolved from RoleAssignmentService", async () => {
    const roleAssignments = { getRolesForUser: () => Promise.resolve(["admin"]) };
    const app = buildTestApp(verifier, roleAssignments);
