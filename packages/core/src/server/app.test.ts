...
describe("GET /metadata/entities/:entity", () => {
  ...
  it("returns projected metadata without leaking the internal Zod schema", async (ctx) => {
    ...
    const response = await app.inject({
      method: "GET",
      url: "/metadata/entities/test.widgets",
      headers: { authorization: `Bearer ${token}` },
    });

    expect(response.statusCode).toBe(200);
    const body = response.json<{ data: Record<string, unknown> }>();
    expect(body.data.name).toBe("test.widgets");
  ...
  it("GET /api/:entity/:id returns the record that was just created", async (ctx) => {
    ...
    try {
      ...
      const getRes = await app.inject({
        method: "GET",
        url: `/api/test.widgets/${created.data.id}`,
        headers: { authorization: `Bearer ${token}` },
      });
