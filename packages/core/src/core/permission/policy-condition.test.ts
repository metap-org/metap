...
describe("evaluateCondition", () => {
  ...
  it("resolves value from context via fromContext", () => {
    ...
    expect(evaluateCondition(condition, { createdBy: "someone-else" }, context)).not.toBe(true);
  });
  ...
  it("requires every condition in 'all' to pass", () => {
    ...
    expect(evaluateCondition(condition, { status: "active", region: "us" }, context)).not.toBe(
      true,
    );
  });
