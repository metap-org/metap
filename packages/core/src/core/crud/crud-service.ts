...
export class CrudService {
  ...
  private maskRecordForRead<
    T extends { code: string | null; status: string | null; data: unknown },
  >(entity: EntityDefinition, context: RequestContext, snapshot: PermissionSnapshot, row: T): T {
    const filteredData = snapshot.filterReadableFields(
      context,
  ...
  async update(
    ...
    const writeDecision = snapshot.assertWritableFields(
      context,
      Object.keys(rawData),
      existingData,
    );
