...
describe("CrudService.update (live DB)", () => {
  ...
  it("rolls back the record insert when the outbox write fails", async (ctx) => {
    ...
    const enqueueSpy = vi.spyOn(container.outbox, "enqueue").mockImplementationOnce(() => {
      throw new Error("simulated outbox failure");
    });

    try {
      await expect(
        container.crud.create("test.widgets", { code: "U005", name: "Rollback Test" }, context),
      ...
      for (const leftoverRow of leftover.rows) {
        await pgClient.query("DELETE FROM outbox_events WHERE aggregate_id = $1", [leftoverRow.id]);
      }
  ...
  it("executes a valid transition: updates state/status/version, logs the event, emits the outbox event", async (ctx) => {
    ...
    try {
      ...
      const outboxRows = await pgClient.query<{ topic: string }>(
        ...
        [created.data.id, "test.widgets.workflow.transitioned"],
      );
...
describe("CrudService field/record enforcement (live DB)", () => {
  ...
  it("returns fieldErrors naming the invalid field(s) on a validation failure", async (ctx) => {
    ...
    const result = await container.crud.create("test.widgets", { code: "" }, adminContext);
  ...
  it("does not empty an admin's list() results because of a non-admin-scoped record-level read policy", async (ctx) => {
    ...
    try {
      const result = await container.crud.list("test.widgets", { limit: 20 }, adminContext);
  ...
  it("get() returns an existing record, field-masked the same way list() would mask it", async (ctx) => {
    ...
    try {
      ...
      const result = await container.crud.get("test.widgets", recordId, viewerContext);
  ...
  it("get() returns 403 for a non-admin blocked by a record-level read policy, but still succeeds for admin", async (ctx) => {
    ...
    try {
      const editorResult = await container.crud.get("test.widgets", created.data.id, editorContext);
      expect(editorResult.ok).toBe(false);
      if (!editorResult.ok) {
        expect(editorResult.status).toBe(403);
      }

      const adminResult = await container.crud.get("test.widgets", created.data.id, adminContext);
  ...
  it("get() capabilities.canUpdate and transitions reflect a record-level update denial", async (ctx) => {
    ...
    try {
      const result = await container.crud.get("test.widgets", created.data.id, editorContext);
      expect(result.ok).toBe(true);
      if (result.ok) {
  ...
  it("delete() soft-deletes a record when the version matches; it then 404s on get()", async (ctx) => {
    ...
    try {
      ...
      const afterDelete = await container.crud.get("test.widgets", created.data.id, adminContext);
  ...
  it("delete() rejects a stale version and leaves the record un-deleted", async (ctx) => {
    ...
    try {
      ...
      const stillThere = await container.crud.get("test.widgets", created.data.id, adminContext);
