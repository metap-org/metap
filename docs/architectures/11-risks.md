...
| `packages/platform-react` hard-depends on `react-router-dom` | `WorkflowActionBar`/`RecordDetail`/`GeneratedForm` call `Link`/`useNavigate` directly — a consumer using a different router (or none) can't use them as-is, despite the package's stated router-agnostic goal | A real consumer needs a different router — navigation would need to become an injected callback/render-prop instead of a direct import |
