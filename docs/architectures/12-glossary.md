...
| Term | Meaning |
|---|---|
| **Entity** | A business object type declared once as an `EntityDefinition` (`*.entity.ts`) — fields, list views, validation schema, workflow. No dedicated database table; stored in the generic `records` table. |
...
| **Outbox pattern** | Writing an event to a DB table in the same transaction as the business change, then draining it to RabbitMQ from a separate process — avoids losing events on broker downtime. |
...
| **`IndexReconciler`** | Reads `EntityField.indexed`/`unique`/`searchMode` and creates matching partial expression / GIN indexes on `records`, idempotently, at boot and via `pnpm index:reconcile`. |
...
| **Workflow transition** | A guarded, atomic state change on an entity's `stateField`, logged to `workflow_events` and emitted as an outbox event. |
