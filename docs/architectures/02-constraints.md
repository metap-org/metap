...
- Module/route code must not import the Drizzle client or RabbitMQ publisher directly — go through `CrudService`/`OutboxService` via the container (`packages/core/src/core/container.ts`).
