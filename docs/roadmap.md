...
| 26. Làm đầy jira-server/jira-fe — auth thật, issue detail+comment, backlog, attachment | Done 2026-08-24 — cả 4/4 bước (auth thật; issue detail+comment; sprint backlog + bug thật fix ở `metap-query`; attachment — consumer thật đầu tiên của `metap-storage`), verify sống đầy đủ mọi bước |
| 27. Generalize attachment thành năng lực platform (`metap-http`) | Done 2026-08-24 — crate `metap-attachments` (2 cơ chế: bảng chung + bảng riêng theo entity), route generic `/api/{entity}/{id}/attachments*`, xoá bespoke cũ khỏi jira-server. Đánh đổi: mất bảo vệ chặn-xoá-khi-còn-attachment (đã verify + ghi nhận). Phát hiện thêm 1 gap migration thật (`0019` không an toàn chạy lại cho dedicated-db tenant đã provision) |
...
- **Gap migration thật phát hiện lúc verify** (chưa fix, ghi nhận): migration `0019` (Phase 25's
  `tenant_auth_configs`) có bước backfill query `control.tenants` — chạy lại `db-migrate` cho 1
  tenant `dedicated_db` **đã provision từ trước** (nên đã tự `DROP SCHEMA control`) sẽ lỗi ngay ở
  migration 19, chặn cả 20/21 chạy theo sau (do `sqlx::migrate!` áp migration tuần tự, 1 cái lỗi
  chặn hết). Không sửa migration 19 (đã chạy thật ở DB platform, sửa nội dung sẽ vỡ checksum
  `sqlx::migrate!` lưu) — áp tay DDL của `0021_attachments.sql` trực tiếp qua `psql` cho
  `metap_myjira` để tiếp tục verify. Rủi ro thật cho bất kỳ tenant `dedicated_db` nào provision
  trước khi 1 migration mới có bước phụ thuộc `control.tenants` được thêm vào — cần nhớ: migration
  chạm `control.*` chỉ an toàn chạy lúc **provision lần đầu** (khi `control` còn tồn tại), không
  an toàn chạy lại qua `db-migrate` cho tenant đã tồn tại.
