...
- clean package boundaries between the reusable core and each business module (`packages/core` + `apps/<module>`), and between the reusable frontend and its demo consumer (`packages/platform-react` + `apps/crm-fe`) — a monorepo shape chosen specifically to keep this direction cheap, not a generic engineering preference
